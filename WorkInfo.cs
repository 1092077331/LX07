    ...
    public class WorkInfo
    {
        ...
        public IEnumerable<Model.WorkInfoNo> GetMyPage(Model.WorkMyPage page)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                ...
                sql += "select* from a where num between (@pageIndex-1)*@pageIndex+1 and @pageIndex*@pageSize;";
                return cn.Query<Model.WorkInfoNo>(sql, page);
            }
        }
        public int Add(Model.WorkInfo workinfo)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "insert info workinfo(workId,workName,workPicture,uploadTime,workIntroduction,workVerify,userName,activityId,recommend,recommendTime) " +
                    "values(@workId,@workName,@workPicture,@uploadTime,@workIntroduction,@workVerify,@userName,@activtyId,@recommend,@recommendTime);";
                sql += "SELECT @@IDENTITY";
